  ...
  <c:forEach var="product" items="${requestScope.productsActive}"> 
    <div class="w3-third w3-container w3-margin-bottom">
      ...
      <div class="w3-container w3-white">
        ...
        <form id="deleteForm/${product.name}/${product.id}" method="POST" action="${contextPath}/seller/products/delete/${product.name}/${product.id}">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
            <button type="submit" class="w3-bar-item w3-button w3-padding">Delete Product</button>
        </form>
        <a href="${contextPath}/seller/products/edit/${product.name}/${product.id}" class="w3-bar-item w3-button w3-padding">Edit Product</a>
      </div>
    </div>
    </c:forEach>
  </div>
